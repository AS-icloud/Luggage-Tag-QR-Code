...
- The ready-to-print back QR is an offline vCard. Contact values are stored once (same Personal Details fields as the online option), with English, Spanish, French and German recovery messages plus the online page URL in the Notes field. It can be read without a live translation service — scanning it prompts the finder's phone straight to "Create New Contact" / "Add to Existing Contact", no internet needed.
- "vCard Contact" is now the default QR Content option for the back, so a fresh label already uses the fully offline vCard. Switch back to "Short web URL only" if you specifically want the back to duplicate the front link instead.
- The front QR's page (l.html) now also has a "Save as Contact" button beneath the two copy buttons, so a finder with internet can get the same native Add-to-Contacts screen from the front, in addition to the language dropdown and copy options.
